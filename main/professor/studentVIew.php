...
// Get all students enrolled in professor's assigned courses
$students = $db->fetchAll(
    "SELECT DISTINCT u.id, u.first_name, u.last_name, u.email,
            ...
            (SELECT COUNT(*) FROM course_grades cg 
             WHERE cg.student_id = u.id AND cg.grade IS NOT NULL AND CAST(cg.grade AS DECIMAL) < 75) as failed_subjects
     FROM users u
     JOIN student_profiles sp ON u.id = sp.user_id
     JOIN course_enrollments ce ON u.id = ce.student_id
     JOIN professor_course_assignments pca ON ce.course_id = pca.course_id
     WHERE u.role = 'student' AND pca.professor_id = ?
